...
public class ExerciseEntryDbHelper extends SQLiteOpenHelper {
	...
	public long insertEntry(ExerciseEntry entry) {
		...
		// GPS track is a little bit different. It's stored as binary data with
		// self-cooked conversion function in Utils.data.

		byte[] byteLocations = entry.getLocationByteArray();
		if (byteLocations.length > 0) {
			value.put(KEY_GPS_DATA, byteLocations);
		}
